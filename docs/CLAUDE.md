...
**Diagnostic tooling added while investigating**: passive slow-request logging (before_request/after_request pair registered first in the file, right after `app = Quart(__name__)`, so the timer wraps the entire request lifecycle including every other hook and the route handler). Logs `SLOW REQUEST: <method> <path> took Xs (status N)` via `app.logger.warning` for anything over `SLOW_REQUEST_THRESHOLD_SECONDS` (default 2.0s). Lets the next occurrence be found via log grep instead of needing to catch it live.  
**Next steps if it recurs**: (1) grep logs for `SLOW REQUEST` first; (2) if not caught by that, hit a trivial route (`/robots.txt`) from a second terminal while `/login` is actively hanging, to tell whether it's a whole-event-loop freeze or specific to `/login`'s code path; (3) re-run the timed curl test against the *correct* IP during an active hang; (4) check whether the delay is constant or periodic (points at a scheduled background job — search crawler, 15-min cleanup scheduler, SMB credential refresh ~30s, file_monitor walk) by repeating the curl back-to-back.  
