...
**Problem** (UNRESOLVED, first seen 2026-09-11): `/login` intermittently takes ~45s–131s to load on Android Chrome's normal (non-incognito) profile only — incognito, desktop Chrome, and other browsers are unaffected. Logging out shows the same delay. Fixed temporarily by clearing the browser's site data, but recurs after a period of normal use.  
**Ruled out** (confirmed NOT the cause, keep this list so it isn't re-investigated from scratch):
  - *HSTS lockout* — a real, confirmed issue existed (`_request_via_trusted_tls()`'s header-based trust check let a dynamic HSTS entry get set even though the self-signed origin can't satisfy it), verified via `chrome://net-internals/#hsts` showing a dynamic entry matching the app's own header. But `files.ccustomdomain.com` (same domain, would be covered by the same `includeSubDomains` policy) loads fine, and the hang persisted even after fully disabling HSTS at the Cloudflare dashboard — so HSTS is not what's causing this specific hang. (The underlying HSTS design issue is still worth fixing at some point — see hostname-separation note below.)
  - *Cloudflare bot-challenge* (`/cdn-cgi/challenge-platform/...` seen in a chrome://net-export capture) — ruled out because the hang reproduces identically on direct LAN IP access, which never touches Cloudflare.
  - *A genuine 131s server-side TTFB stall* — a `curl -w` timing test appeared to show `connect`/`tls` ≈0s but `ttfb` ≈131s, which looked like a confirmed blocking call inside app.py/Hypercorn. This was later found to have been run against the wrong IP address by mistake — invalid data, not a real finding. Whether a genuine server-side stall exists is still an open question and needs to be re-tested properly (timed curl against the *correct* IP, during an actual active hang) if this recurs.
**Still valid / not yet fixed**:
  - `/login` (GET) and `/logout` both unconditionally send `Clear-Site-Data` (`"cache"` on login, `"cache", "cookies", "storage"` on logout) whenever `_request_is_secure()` — forcing the browser to wipe its entire origin disk cache synchronously on every anonymous visit. This is a real, unnecessarily destructive bug independent of the hang's root cause (a login-page view doesn't need to nuke cached CSS/JS/fonts/media-preview assets), and plausibly a *contributing* factor even if not the sole cause: a normal profile's accumulated cache takes real, scaling time to wipe, while incognito has nothing to wipe. Not patched yet.
**Diagnostic tooling added while investigating**: request logging (before_request/after_request pair registered first in the file, right after `app = Quart(__name__)`, so the timer wraps the entire request lifecycle including every other hook and the route handler). Logs **every** request, not just slow ones — `<method> <path> took X.XXXs (status N)` at INFO — escalating to `SLOW REQUEST: ...` at WARNING for anything over `SLOW_REQUEST_THRESHOLD_SECONDS` (default 2.0s). Logging every request (not just slow ones) is deliberate: it lets a recurrence be correlated against everything else happening at the same timestamp (another route also going slow, a periodic background job, etc.). Uses its own logger (`cloudinatorftp.requests`) with the project's standard timestamp format (`"%(asctime)s [%(levelname)s] %(message)s"`, matching `prod_server.py`'s `_build_hypercorn_logger`) rather than Quart's default `app.logger` — the default format's `%(module)s` field is what caused a cosmetic `(unknown file)` glitch, same class of issue as the Python 3.14 `%(process)d` bug already documented above. Writes to **both** the console and a persistent `logs/requests.log` (created next to `app.py`), so entries survive even when the server runs detached (systemd/Docker/screen/tmux) and nobody was watching the console live. Lets the next occurrence be found via `grep "SLOW REQUEST" logs/requests.log` instead of needing to catch it live.  
**Next steps if it recurs**: (1) grep `logs/requests.log` for `SLOW REQUEST` first — note whether other routes were also slow at the same timestamp (whole-event-loop freeze) or only `/login`/`/logout` (route-specific); (2) if not caught by that, hit a trivial route (`/robots.txt`) from a second terminal while `/login` is actively hanging; (3) re-run the timed curl test against the *correct* IP during an active hang; (4) check whether the delay is constant or periodic (points at a scheduled background job — search crawler, 15-min cleanup scheduler, SMB credential refresh ~30s, file_monitor walk) by repeating the curl back-to-back.  
**Longer-term fix, unrelated to root-causing the hang itself**: reserve the public hostname exclusively for the Cloudflare-fronted trusted-cert path; never let the self-signed origin answer under that same hostname (use the Tailscale `*.ts.net` hostname for direct/LAN access instead). This closes off the HSTS design issue permanently regardless of what's causing the current hang.
