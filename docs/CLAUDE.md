...
- **Where `ip=` comes from**: `REMOTE_ADDR`, which asgiref fills from Hypercorn's ASGI `scope["client"]` (the real TCP peer; falls back to `?` if absent). `X-Forwarded-For`/`CF-Connecting-IP` are **not** read — unlike the web UI's `get_client_ip()` (see the 2026-09-23 sync note). That's correct for a directly exposed listener (the headers would be client-spoofable), but it means **behind any proxy/tunnel the logged IP is the proxy's**. Nothing in this repo's docs says WebDAV is tunneled; if it ever is, this needs a trusted-header path like `get_client_ip()`'s.
